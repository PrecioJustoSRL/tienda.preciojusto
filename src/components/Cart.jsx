...
export default function Button({ theme, styled, click, children }) {
    ...
    const { user, cart, tienda, productDB, setUserProduct, setUserItem, setUserSuccess, success, setNav } = useUser()
    ...
    const redirectHandlerWindow = () => {
        window.open(`https://api.whatsapp.com/send?phone=+59169941749&text=hola%20necesito%20un%20implante%20de%20osteosintesis%20¿Pueden%20ayudarme?%20`, '_blank')
        setNav(false)
    ...
    return (<div className={` transition-all ease-in-out duration-300  z-40`}>
        <button type="button" onClick={redirectHandlerWindow} className="h-[40px] w-[40px] my-1 text-blue-700 bg-[#2A52BE] border-[2px] border-white font-medium rounded-full text-sm p-2.5 text-center flex justify-center items-center ">
            {tienda !== 'Recetar'
